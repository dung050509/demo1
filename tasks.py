...
collection_nft_wrapped = dbname["nft_wrapped"]
...
@celery_app.task()
def extract_ntf_wrapped():
    """A pointless Celery task to demonstrate usage."""
    tweets = []
    try:
        for i, tweet in enumerate(
            snstwitter.TwitterSearchScraper(query="(#NFTWrapped2022) (@TeaDAO_money)").get_items()
        ):
            tweet = json.dumps(tweet.__dict__, default=datetime_handler)
            print(tweet)
            tweets.append(json.loads(tweet))
    ...
    try:
        collection_nft_wrapped.insert_many(tweets, bypass_document_validation=True)
    except BulkWriteError as e:
        print("error", e)
