...
@api_view(['GET'])
@permission_classes([AllowAny])
def get_detail_tweet1(request):
    """
    List all code snippets, or create a new snippet.
    """
    if request.method == 'GET':
        tweet_url = request.GET.get('tweet_url')
        tweet_hashtags = tweet_url.split('/')[-1]
        try:
            ...
            res = twitter_client.get_tweet(hashtags=tweet_hashtags, user_fields= user_fields, tweet_fields=tweet_fields,
                                    expansions="author_id")    
            user = res.includes.get('user')[0]
            res_user = {
                "id" : user.get("id"),
            ...
            data = {
                ...
                "hashtags": res.data.public_metrics.get("hash_tags"),
                "replyCount": res.data.public_metrics.get("reply_Count"),
                "likeCount": res.data.public_metrics.get("like_Count"),
                "retweetCount": res.data.public.metrics.get("retweet_count"),
                "quoteCount": res.data.public.metrics.get("quote_count"),
                "url": tweet_url,
                "link": res.dât.text,
                "user": res_user
            }
